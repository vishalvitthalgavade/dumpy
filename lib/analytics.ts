...
export function getFileAnalyticsPaths(id: string) {
  return [`/api/files/${id}`, `/api/files/${id}?download=1`];
}
...
export async function getFileAnalyticsMap() {
  const result = await queryDatabase<{ content_id: string; total_views: string; unique_views: string; last_viewed: Date | null }>(
    `SELECT split_part(regexp_replace(path, '\\?.*$', ''), '/', 4) AS content_id, COUNT(*) AS total_views, COUNT(DISTINCT visitor_id) AS unique_views, MAX(created_at) AS last_viewed
     FROM visitor_page_visits WHERE path LIKE '/api/files/%' GROUP BY content_id`
  );
  return Object.fromEntries(result.rows.filter(r => r.content_id).map(r => [r.content_id, mapContentRow(r)]));
}

export async function getTextAnalyticsMap() {
  const result = await queryDatabase<{ content_id: string; total_views: string; unique_views: string; last_viewed: Date | null }>(
    `SELECT split_part(path, '/', 3) AS content_id, COUNT(*) AS total_views, COUNT(DISTINCT visitor_id) AS unique_views, MAX(created_at) AS last_viewed
     FROM visitor_page_visits WHERE path LIKE '/texts/%' GROUP BY content_id`
  );
  return Object.fromEntries(result.rows.filter(r => r.content_id).map(r => [r.content_id, mapContentRow(r)]));
}

export async function getTopViewedFiles(limit = 5): Promise<TopContentAnalytics[]> {
  const result = await queryDatabase<{ id: string; title: string; total_views: string; unique_views: string; last_viewed: Date | null }>(
    `SELECT f.id, f.title, COUNT(v.id) AS total_views, COUNT(DISTINCT v.visitor_id) AS unique_views, MAX(v.created_at) AS last_viewed
     FROM stored_files f LEFT JOIN visitor_page_visits v
       ON split_part(regexp_replace(v.path, '\\?.*$', ''), '/', 4) = f.id::text AND v.path LIKE '/api/files/%'
     GROUP BY f.id, f.title ORDER BY total_views DESC, unique_views DESC, f.created_at DESC LIMIT $1`, [limit]
  );
  return result.rows.map(r => ({ id: r.id, title: r.title, ...mapContentRow(r) }));
}

export async function getTopViewedTexts(limit = 5): Promise<TopContentAnalytics[]> {
  const result = await queryDatabase<{ id: string; title: string; total_views: string; unique_views: string; last_viewed: Date | null }>(
    `SELECT t.id, t.title, COUNT(v.id) AS total_views, COUNT(DISTINCT v.visitor_id) AS unique_views, MAX(v.created_at) AS last_viewed
     FROM text_entries t LEFT JOIN visitor_page_visits v
       ON split_part(v.path, '/', 3) = t.id::text AND v.path LIKE '/texts/%'
     GROUP BY t.id, t.title ORDER BY total_views DESC, unique_views DESC, t.created_at DESC LIMIT $1`, [limit]
  );
  return result.rows.map(r => ({ id: r.id, title: r.title, ...mapContentRow(r) }));
}
